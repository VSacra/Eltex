...
Contact* addContact(Contact NewContact) {
	if(!PhoneBook){ PhoneBook = (Contact*)malloc(NUMBER_TO_ADDS * sizeof(Contact)); }
	N++;
	if (N % NUMBER_TO_ADDS == 0) {
		Contact* new_PhoneBook = NULL;
		new_PhoneBook = (Contact*)realloc(PhoneBook, (N+NUMBER_TO_ADDS) * sizeof(Contact));
		if (new_PhoneBook == NULL) {
			free(PhoneBook); return NULL;
		}
		PhoneBook = new_PhoneBook;
	}
	PhoneBook[N-1] = NewContact;
	AddID(&PhoneBook[N-1]);
	return &PhoneBook[N - 1];
}
...
Contact* editContact(int id, int k, char str[10], ...) {
	...
	int num = -1;
	for (int i = 0; i < N; i++) {
		if (PhoneBook[i].ID == id) { num = i; break; }
	}
	...
	for (int i = 0; i < k; i++){
		...
		case 'i': {
			...
			free(PhoneBook[num].Imya);
			PhoneBook[num].Imya = (char*)malloc(strlen(im) + 1);
			strcpy(PhoneBook[num].Imya, im);
			break; }
		case 'f': {
			char* fa = va_arg(factor, char*);
			free(PhoneBook[num].Familiya);
			PhoneBook[num].Familiya = (char*)malloc(strlen(fa) + 1);
			strcpy(PhoneBook[num].Familiya, fa);
			break; }
		case 'o': {
			char* ot = va_arg(factor, char*);
			if(PhoneBook[num].Otchestvo)free(PhoneBook[num].Otchestvo);
			PhoneBook[num].Otchestvo = (char*)malloc(strlen(ot) + 1);
			strcpy(PhoneBook[num].Otchestvo, ot);
			break; }
		case 'n': {
			...
			for (int j = 0; j < kNum && j < MAX_PHONES; j++) {
				strncpy(PhoneBook[num].Numbers[j], numArr + j * MAX_STRING, MAX_STRING);
				PhoneBook[num].Numbers[j][MAX_STRING - 1] = '\0';
			}
			for (int j = kNum; j < MAX_PHONES; j++) {
				PhoneBook[num].Numbers[j][0] = '\0';
			}
			break;
		}
		case 's': {
			...
			if (soc) {
				// Копируем соцсети
				strncpy(PhoneBook[num].Soc.VK, soc->VK, MAX_STRING);
				strncpy(PhoneBook[num].Soc.OK, soc->OK, MAX_STRING);
				strncpy(PhoneBook[num].Soc.TG, soc->TG, MAX_STRING);

				PhoneBook[num].Soc.VK[MAX_STRING - 1] = '\0';
				PhoneBook[num].Soc.OK[MAX_STRING - 1] = '\0';
				PhoneBook[num].Soc.TG[MAX_STRING - 1] = '\0';
			}
			break; }
...
int deleteContact(int id) {
	bool find = 0;
	for (int i = 0; i < N; i++) {
		if (PhoneBook[i].ID == id) find = 1;
		if (find) PhoneBook[i] = PhoneBook[i + 1];
	}
	if (!find) return -1;
	N--;
	return 0;
...
void AddID(Contact* new_id) {
	...
	while (Deny != 0) {
		id = rand ();
		for (int i = 0; i < N; i++) {
			if (PhoneBook[i].ID == id) { Deny = 1; break; };
		}
		Deny = 0;
	}
	new_id->ID = id;
...
void editConsole() {
	...
	int c;
	while ((c = getchar()) != '\n' && c != EOF) {}
	if (PhoneBook[Num - 1].ID == -1) printf("\nОшибка. Нет такого номера.");
	Net Soc = PhoneBook[Num - 1].Soc;
	id = PhoneBook[Num - 1].ID;
...
void deleteConsole() {
	...
	if (deleteContact(PhoneBook[num - 1].ID) == 0) printf("\nКонтакт был успешно удалён\n");
...
void View() {
	...
	for (int i = 0; i < N; i++) {
		...
		// ФИО с пробелами
		printf("%s ", PhoneBook[i].Imya);
	    printf("%s ", PhoneBook[i].Familiya);
		if (PhoneBook[i].Otchestvo) printf("%s", PhoneBook[i].Otchestvo);
		printf("\n");
		...
		for (int j = 0; j < MAX_PHONES; j++) {
			if (PhoneBook[i].Numbers[j][0] != '\0') { // Проверяем что номер не пустой
				if (hasPhones) printf(", ");
				printf("%s", PhoneBook[i].Numbers[j]); // Выводим строку целиком
				hasPhones = 1;
			}
		}
		printf("\n");
		...
		printf("Соцсети: ");
		int hasSocial = 0;
		if (PhoneBook[i].Soc.VK[0] != '\0') {
			printf("VK: %s", PhoneBook[i].Soc.VK);
			hasSocial = 1;
		}
		if (PhoneBook[i].Soc.OK[0] != '\0') {
			if (hasSocial) printf(", ");
			printf("OK: %s", PhoneBook[i].Soc.OK);
			hasSocial = 1;
		}
		if (PhoneBook[i].Soc.TG[0] != '\0') {
			if (hasSocial) printf(", ");
			printf("TG: %s", PhoneBook[i].Soc.TG);
			hasSocial = 1;
		}
